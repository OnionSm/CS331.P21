...
class VAE_Encoder(nn.Sequential):
    def __init__(self):
        super.__init__(
            ...
            VAE_ResidualBlock(128, 256), # [B, 128, H/2, W/2]
            VAE_ResidualBlock(256, 256), # [B, 128, H/2, W/2]
    ...
    def forward(self, x: torch.Tensor, noise: torch.Tensor) -> torch.Tensor:
        '''
        Input: x: [B, C, H, W], noise: (B, 8, H/8, W/8)


        '''
        for module in self:
            if getattr(module, "stride", None) == (2,2):
                x = F.pad(x, (0, 1, 0, 1)) # Add top and bottom
            x = module(x) 
        
        mean, log_variance = torch.chunk(x, 2, dim=1) # 2 tensor([B, 4, H/8, W/8])
        log_variance = torch.clamp(log_variance, -30, 20) # [B, 4, H/8, W/8]
        variance = log_variance.exp() # [B, 4, H/8, W/8]
        stdev = variance.sqrt() # [B, 4, H/8, W/8] 
        x = mean + stdev * noise
        x *= 0.18215
        return x
